...
import TouchableIcon, { CleanIconsType } from "@components/interactions/TouchableIcon";
	...
	return (
		<div className="fixed bottom-0 left-0 w-full sm:relative">
			<nav className="box m-3.5 flex h-14 flex-row justify-around rounded-xl border-[1px] py-[5px] sm:hidden">
				{links.map((l) => {
					const isActive = l.href.length != 1 ? pathname.startsWith(l.href) : pathname == l.href;
					...
					return (
						...
						>
							...
							<p className="text-[10px] font-extralight text-current">{l.name}</p>
						</Link>
					);
				})}
			</nav>
